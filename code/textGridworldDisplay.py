...
def indent(rows, hasHeader=False, headerChar='-', delim=' | ', justify='left',
    ...
    def rowWrapper(row):
        ...
        mappedRowElements = list(map(lambda x : x, itertools.zip_longest(*newRows)))
        return [[substr or '' for substr in item] for item in mappedRowElements]
    # break each logical row into one or more physical ones
    logicalRows = [rowWrapper(row) for row in rows]
    # columns of physical rows
    columns = list(map(lambda x : x, itertools.zip_longest(*reduce(operator.add,logicalRows))))
